<?php
///////////////////// KONEKSI KE DATABASE //////////////////////////////
$db = mysqli_connect("localhost", "root", "", "sirusakdeh_db");
...
function query($query) {
  ...
  $rows = []; // Mendefinisikan $rows sebagai array kosong sebelum perulangan
...
$query = "INSERT INTO jadwal
            VALUES ('', '$dokter','$level', '$poli','$senin','$selasa','$rabu','$kamis','$jumat','$sabtu')
          ";
...
$query = "INSERT INTO pasien
            VALUES ('','$username','$tgl_daftar','$id_poli','$id_dokter','$id_jam','$jenis_pembayaran','$no_rm','$nama','$jk','$tlahir','$nohp','$ttl','$pekerjaan','$alamat','$wnama','$wjk','$walamat','$wnohp','$status','$diagnosa')
          ";
...
function validasi($data) {
  ...
  // Pastikan ada tanda kutip tunggal di sekitar nilai yang berupa string
  $query = "UPDATE pasien SET
            status = '$status',
            diagnosa = '$diagnosa'
            -- no_rm = '$no_rm'
...
function tambah_dokter($data) {
  ...
  $id_poli = htmlspecialchars($data["poli_tujuan"]);
  $status = "1";
  $level = htmlspecialchars($data["level_dokter"]);

//TAMBAH dokter KE DATABASE
$query = "INSERT INTO dokter
            VALUES ('','$nama_dokter', '$id_poli','$status','$level')
          ";
...
function cari_dokter($dokter) {
  $query = "SELECT dokter.id_dokter,dokter.nama_dokter,poliklinik.nama_poli,jam.jam,dokter.level from dokter join poliklinik on dokter.id_poli=poliklinik.id_poli join jam on jam.id_jam=jam.id_jam
              WHERE
              nama_dokter LIKE  '%$dokter%'GROUP BY dokter.id_dokter";
...
//   $query = "SELECT dokter.id_dokter, dokter.nama_dokter, poliklinik.nama_poli, jam.jam, dokter.level
//             FROM dokter 
//             JOIN poliklinik ON dokter.id_poli = poliklinik.id_poli 
//             JOIN jam ON jam.id_jam = jam.id_jam
//             WHERE dokter.nama_dokter LIKE '%$dokter%'";
